...
const EditUserUniversitySection = ({ usersUniversity }: { usersUniversity?: UsersUniversity }) => {
  ...
  const methods = useForm<EditUserUniversityModel>({
    resolver: yupResolver(EditUserUniversitySchema),
    defaultValues: {
      firstname: usersUniversity?.firstname || '',
      lastname: usersUniversity?.lastname || '',
      email: usersUniversity?.email || '',
      gender: usersUniversity?.gender || GENDER.NOT_SPECIFIED,
      DOB: usersUniversity?.DOB.toString() || '',
      phoneNumber: usersUniversity?.phoneNumber || '',
      address: usersUniversity?.address || '',
  ...
  return (
    <div className="w-[60%] p-6 m-auto bg-white rounded-md shadow-md ring-2 ring-gray-800/50 max-md:w-[100%]">
      <h1 className="text-3xl font-semibold text-center text-gray-700">แก้ไขข้อมูลสมาชิก</h1>
      <FormProvider {...methods}>
        <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div>
            <button className="btn btn-block btn-info">แก้ไขข้อมูลสมาชิก</button>
          </div>
        </form>
      </FormProvider>
    </div>
...
export default EditUserUniversitySection
